...
bool is_overlapping(entry e1, entry e2) {
    ...
    return get_minute_of_day(e1.start) < get_minute_of_day(e2.end)
        && get_minute_of_day(e2.start) < get_minute_of_day(e1.end);
}
    ...
    for (int i = 0; i < n1; i++) {
        for (int j = 0; j < n2; j++) {
            if (is_overlapping(entries1[i], entries2[j])) {
                int s1 = get_minute_of_day(entries1[i].start);
                int s2 = get_minute_of_day(entries2[j].start);

                int e1 = get_minute_of_day(entries1[i].end);
                int e2 = get_minute_of_day(entries2[j].end);
...
// note: There is no mistake in the main method.
int main(void) {
int tc_num = 1;
{
entry entries1[] = {
...
printf("Expected:195\n");
printf("Actual :%d\n",
get_contact_time(entries1, sizeof(entries1)/sizeof(entry),
entries2, sizeof(entries2)/sizeof(entry)));
